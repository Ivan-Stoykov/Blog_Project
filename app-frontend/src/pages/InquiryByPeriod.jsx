...
export default function InquiryByPeriod({
  ...
  return (
    <div className={cardClasses}>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 w-full lg:w-3/5 xl:w-1/2">
          <input
            ...
            name="period"
            title="Start Date"
            className="p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 w-full sm:w-1/3 text-sm"
            ref={field1}
          />
          <input
            type="date"
          ...
          <button
            value="Find"
            className="w-full sm:w-1/3 px-4 py-3 bg-blue-600 text-white font-medium rounded-lg shadow-md hover:bg-blue-700 transition duration-150 flex items-center justify-center space-x-2"
            onClick={() => {
              setPeriod(field1.current.value + "|" + field2.current.value);
            }}
          >
            <svg
      ...
      {posts.length == 0 && (
        <p className="text-center py-8 text-gray-500">Enter period!</p>
      )}
      {posts.length > 0 && (
        <>
